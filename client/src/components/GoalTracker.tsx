...
export function GoalTracker() {
  ...
  // Weights from server are already in lbs — no conversion needed
  const fmt = (lbs: number) => lbs.toFixed(1);
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-lg bg-white/5 border border-white/10">
              ...
              <p className="text-2xl font-bold text-cyan-400">{fmt(goalProgress.currentWeight)} lbs</p>
            </div>

            <div className="p-4 rounded-lg bg-white/5 border border-white/10">
              <p className="text-slate-400 text-sm mb-1">Goal Weight</p>
              <p className="text-2xl font-bold text-green-400">{fmt(goalProgress.goalWeight)} lbs</p>
            </div>

            <div className="p-4 rounded-lg bg-white/5 border border-white/10">
              <p className="text-slate-400 text-sm mb-1">Weight {isWeightLoss ? "Lost" : "Gained"}</p>
              <p className="text-2xl font-bold text-blue-400">{fmt(Math.abs(goalProgress.weightLost))} lbs</p>
            </div>

            <div className="p-4 rounded-lg bg-white/5 border border-white/10">
              <p className="text-slate-400 text-sm mb-1">To Go</p>
              <p className="text-2xl font-bold text-yellow-400">{fmt(goalProgress.weightToGo)} lbs</p>
            </div>
          </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-white/5 border border-white/10">
              ...
              <p className="text-3xl font-bold text-cyan-400">{Math.abs(goalProgress.weeklyWeightChangeRate).toFixed(2)} lbs</p>
        ...
        <CardContent className="space-y-3">
          ...
          {goalProgress.weeklyWeightChangeRate > 0 && isWeightLoss && (
            <div className="p-3 rounded-lg bg-blue-900/20 border border-blue-500/30">
              ...
              <p className="text-blue-300 text-xs mt-1">You're losing {Math.abs(goalProgress.weeklyWeightChangeRate).toFixed(2)} lbs per week on average.</p>
            </div>
          )}
